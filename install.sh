...
install_base_tools() {
  ...
  if [[ "$OS" == "macos" ]]; then
    ...
    brew install --quiet fzf ripgrep fd jq tree unzip starship
  else
    sudo apt-get install -y git curl wget build-essential fzf ripgrep fd-find jq tree unzip
...
main() {
  ...
  echo "  - OpenCode (AI 助手)"
  echo "  - UV (Python 包管理器)"
  echo "  - 其他开发工具 (fzf, ripgrep, fd, eza 等)"
  echo ""
